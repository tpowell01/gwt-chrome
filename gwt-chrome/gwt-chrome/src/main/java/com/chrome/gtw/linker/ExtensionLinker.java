...
import com.chrome.gtw.json.serialization.JsonArray;
import com.chrome.gtw.json.serialization.JsonObject;
import com.chrome.gtw.json.serialization.JsonValue;
import com.chrome.gtw.linker.ExtensionArtifact.IconInfo;
...
public class ExtensionLinker extends AbstractLinker {
  ...
  public ArtifactSet link(TreeLogger logger, LinkerContext context,
    ...
    String optionPage = extension.getOptionPage();
    return addArtifacts(artifacts, emitString(logger, generateHtmlContents(
        ...
        generateManifestContents(logger, backgroundPageFileName, optionPage, extension,
  ...
  private JsonObject createBrowserAction(
    ...
    if (browserAction.getPopup().length() > 0) {
        browserActionObject.put("popup", browserAction.getPopup());
    }
  ...
  private String generateManifestContents(TreeLogger logger,
      String backgroundPageFileName, String optionPageFileName ,ExtensionArtifact extension,
      SortedSet<PageActionArtifact> pageActions,
      SortedSet<BrowserActionArtifact> browserActions,
    ...
    if (!optionPageFileName.equals(Extension.NO_OPTION_PAGE)) {
        config.put("options_page", optionPageFileName);
      }
